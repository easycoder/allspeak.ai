# AllSpeak Python Primer (Outline)

Use this primer when building AllSpeak applications that run on the Python CLI runtime (`allspeak-py`).

## 1) Purpose

Establish a safe, milestone-first workflow for Python AllSpeak scripts, with strict syntax discipline and explicit verification.
...
For JSON/data access, use built-in AllSpeak JSON commands and verify with small sample data first.
...
## 10) If Project Uses Both JS and Python AllSpeak
