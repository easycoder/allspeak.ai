...
function parseArgs() {
  const args = { conformanceRoot: 'conformance', distPath: 'dist/allspeak.js', output: 'conformance/actuals-js-browser.json' };
...
function buildSandbox(logCollector, errorCollector) {
  // AllSpeak.js assigns window.onload and expects window.AllSpeak
...
function runTest(distCode, scriptSource) {
  ...
  // We'll override after AllSpeak boots inside the VM
  const vmContext = vm.createContext(sandbox);

  // Inject the dist bundle, then expose the AllSpeak const into the sandbox
  // (bundle uses `const AllSpeak = {...}` which doesn't bind to vmContext keys)
  try {
    vm.runInContext(distCode + '\n__EC__ = AllSpeak;', vmContext);
  } catch (e) {
    return { logs: [], error: { category: 'runtime', message: `Bundle load failed: ${e.message}` } };
  }

  // AllSpeak.js sets window.onload; the bundle ends with `window.onload = AllSpeak_Startup`
  // We need to patch writeToDebugConsole before startup so log captures work.
  const EC = vmContext.__EC__ || vmContext.window.AllSpeak;
  if (!EC) {
    return { logs: [], error: { category: 'runtime', message: 'AllSpeak object not found after bundle load' } };
  }
  ...
  // Run the test script directly through AllSpeak.start
  try {
