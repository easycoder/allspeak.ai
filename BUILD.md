...
| `starter/<lang>/*` or repo-root `asedit.as` / `asedit.json` | `./build-starters` — and refresh any extracted starter directory (`edit.html`, `asedit.as`), since a running server serves the copy on disk, not the repository |
