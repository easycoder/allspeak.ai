# AllSpeak + Webson Minimal Primer

Use this as the complete reference when helping with an AllSpeak/Webson project.

## 0) Non-negotiable rule

If you are unsure about AllSpeak or Webson syntax, ask the user. Do not guess.
...
AllSpeak supports membership tests using `includes` in conditions:
...
- "Please confirm the canonical AllSpeak commands for: string length, getting the Nth character, and checking whether a character is in a string (for example `AEIOU`)."
...
Webson is JSON UI. In Webson projects: define UI in JSON, `render`, then `attach` IDs in AllSpeak.
...
Before returning AllSpeak code, verify:
