const AllSpeak_Showdown = {

	name: `AllSpeak_Showdown`,
	runtimeProgram: null,
	...
	getValue: (program, name) => {
		const symbol = AllSpeak_Showdown.getSymbol(program, name);
	...
	fallbackTransform: (program, payload) => {
		if (payload === `ec`) {
			return AllSpeak_Showdown.getValue(program, `ECPayload`) || `<strong>AllSpeak</strong>`;
		}
		...
		if (payload.startsWith(`step`)) {
			return `<pre>${AllSpeak_Showdown.getValue(program, `Fragment`)}</pre>`;
		}
		...
		if (payload.startsWith(`link:`)) {
			...
			if (pos > -1) {
				...
				const linkCount = parseInt(AllSpeak_Showdown.getValue(program, `LinkCount`) || `0`, 10) || 0;
				AllSpeak_Showdown.setValue(program, `LinkCount`, String(linkCount + 1));
		...
		run: program => {
			...
			if (typeof showdown === `undefined`) {
				program.require(`js`, `/dist/vendor/showdown/showdown.min.js`,
					() => {
						AllSpeak_Showdown.runtimeProgram = program;
						AllSpeak_Showdown.setupExtension();
	...
	setupExtension: () => {
		const program = AllSpeak_Showdown.runtimeProgram;
		if (!program) {
			return;
		}
	...
	run: program => {
		const command = program[program.pc];
		const handler = AllSpeak_Showdown.getHandler(command.keyword);
		if (!handler) {
...
AllSpeak.domain.showdown = AllSpeak_Showdown;
