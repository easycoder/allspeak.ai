const AllSpeak_Float = {
	...
	getHandler: (name) => {
		switch (name) {
		case `add`:
			return AllSpeak_Float.Add;
		case `divide`:
			return AllSpeak_Float.Divide;
		case `multiply`:
			return AllSpeak_Float.Multiply;
		case `subtract`:
		case `take`:
			return AllSpeak_Float.Take;
		default:
			return null;
		}
	},
	...
	run: program => {
		const command = program[program.pc];
		const handler = AllSpeak_Float.getHandler(command.keyword);
		if (!handler) {
...
AllSpeak.domain.float = AllSpeak_Float;
