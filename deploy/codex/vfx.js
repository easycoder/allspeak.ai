const AllSpeak_VFX = {
	...
	getHandler: (name) => {
		...
		case `create`:
			return AllSpeak_VFX.Create;
		case `on`:
			return AllSpeak_VFX.On;
		case `set`:
			return AllSpeak_VFX.Set;
		case `start`:
			return AllSpeak_VFX.Start;
		case `step`:
			return AllSpeak_VFX.Step;
		default:
			return null;
		}
	},
	...
	run: program => {
		const command = program[program.pc];
		const handler = AllSpeak_VFX.getHandler(command.keyword);
		if (!handler) {
...
AllSpeak.domain.vfx = AllSpeak_VFX;
