...
Four consequences follow from the domain model:
...
4. **Performance through native escape-hatches.** Hot-path code can be a plugin written in JS or Python, running at full native speed, while the bulk of the application stays in readable AllSpeak. The result approaches the performance of an all-native build with substantially better readability. See [plugins](plugins.md).
