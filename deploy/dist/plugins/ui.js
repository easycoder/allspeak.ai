const AllSpeak_UI = {
	...
	renderDate: (dateRecord) => {
		...
		AllSpeak_UI.monthNames.forEach(function (month, index) {
	...
	Create: {
		...
		run: (program) => {
			...
			case `date`:
				...
				AllSpeak_UI.renderDate(dateRecord);
				...
				dayList.onchange = function () {
					...
					AllSpeak_UI.renderDate(this.dateRecord);
				};
	...
	getHandler: (name) => {
		switch (name) {
		case `create`:
			return AllSpeak_UI.Create;
		case `date`:
			return AllSpeak_UI.Date;
		case `set`:
			return AllSpeak_UI.Set;
		default:
			return null;
		}
	},
	...
	run: (program) => {
		const command = program[program.pc];
		const handler = AllSpeak_UI.getHandler(command.keyword);
		if (!handler) {
			...
			case `date`:
				...
				case `date`:
					return {
						type: `constant`,
						numeric: false,
						content: `${day} ${AllSpeak_UI.monthNames[month]} ${year}`
					};
				case `timestamp`:
					return {
						type: `constant`,
...
AllSpeak.domain.ui = AllSpeak_UI;
