# AllSpeak Coding Guidelines

This is a live document and must be read by any agent preparing to work on AllSpeak.
...
- Never embed HTML directly in AllSpeak script logic. Use Webson for UI structure.

## Control-flow syntax guard

- AllSpeak block loops use `while ... begin ... end`.
...
- Do not invent non-AllSpeak forms such as `put element N of Array into Value`.
...
- In AllSpeak, variables are global within a script. If you need private working state, move that logic into a separate module script (and a Webson file if needed).
...
- Before adding custom functionality, check whether the behavior already exists in AllSpeak core commands or in an existing plugin.
- Create a plugin when AllSpeak syntax is clumsy for a task, or when wrapping substantial external JavaScript functionality.
...
- Use the standard AllSpeak layout convention: indent normal script lines by one tab so labels can sit on the left margin.
