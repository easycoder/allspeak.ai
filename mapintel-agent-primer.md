...
1. AllSpeak loop blocks use `while ... begin ... end`.
...
Use URL-based runtime loading by default for empty-workspace bootstrap. Relative `js/allspeak/...` paths are valid only when those files are present locally.
...
- This is the expected AllSpeak training pattern.
...
## 2E) AllSpeak Array Access Requirement
...
## 2F) AllSpeak Formatting Convention
...
- Behavior, events, and state machine logic belong in AllSpeak scripts.
...
3. Webson render path is wired from AllSpeak.
