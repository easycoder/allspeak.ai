# AllSpeak Developer Resources: Complete Index

A comprehensive guide to all documentation, patterns, and references for working with AllSpeak's syntax, plugin architecture, and development practices.
...
- `plugins/as_keyboard.py` — Event-driven plugin
...
1. Implement `k_{keyword}` (compile) and `r_{keyword}` (runtime) in `as_core.py`
...
→ See examples in `as_graphics.py` k_set, k_remove
...
├── allspeak/
│   ├── as_compiler.py                 Compiler (skipArticles here)
│   ├── as_handler.py                  Handler base class
│   ├── as_core.py                     Core keywords
│   ├── as_graphics.py                 Graphics keywords
...
**Maintained By**: AllSpeak Development Team  
