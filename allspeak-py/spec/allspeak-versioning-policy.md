# AllSpeak Versioning And Release Policy (Draft 0.1)
...
AllSpeak uses three version streams that must be recorded together in release
notes.

1. Language spec version
- Format: `MAJOR.MINOR`
- Source of truth: `spec/allspeak-language-contract.md`
- Bump `MINOR` for backward-compatible additions.
- Bump `MAJOR` for breaking semantic changes.

2. Runtime version
- JS browser runtime: `AllSpeak.version` in `dist/allspeak.js`.
